// ==UserScript==
// @name        Voat Enhancement Suite Lite
// @author      Horza
// @date        24 june 2015
...
$(window).ready(function () {
    ...
    if ($.inArray(data.currentPageType, ["subverses", "sets", "user", "user-manage", "mysets"]) == -1)
    {
        AppendImageButton();
    }
...
function GetSubversesList() {
    var subversesArr = GM_getValue("Voat_Subverses", "newsubverses,introductions,news");
...
function DisplayCustomSubversesList() {
    ...
    for (var idx in subArr) {
        ...
        if (data.regExpSet.test(subArr[idx])) { //ex: name:12
            ...
            //use a special style for set shortcuts: bold or italic.
...
$(window).scroll(function () {
    ...
    if ($(window).scrollTop() + (data.option.FixedListHeader ? data.option.ListHeaderHeight : 0) > headerAccountPos) {
        $('#header-account').css('position', 'fixed')
                            .css('top', data.option.FixedListHeader ? data.option.ListHeaderHeight : "0")
                            .css('right', '0')
...
$(window).scroll(function () {
    ...
    if ($(window).scrollTop() > headerListPos.top) {
        $('.width-clip').css('position', 'fixed')
                            ...
                            .css("height", data.option.ListHeaderHeight + "px")
...
function ShowUserTag() {
    ...
    //All mention on an username as a link.
    var sel = /\/user\/[^/]*\/?$/i;
    ...
    //if ($.inArray(data.currentPageType, ["user", "user-comments", "users-submissions"]) >= 0) {}

}

function UpdateUserTag(name, tag) {
    $("a[class*='GM_UserTag'][id*='" + name + "']").each(function () {
        $(this).text(tag);
    });
}
...
$(document).on("click", ".GM_UserTag", function () {
    ...
    var newTag = prompt(username, oldTag !== "+" ? oldTag : "").replace(/[:,]/g,"-");
    if (newTag.length > 0) {
        if (oldTag !== "+") {
